...
plan boltello::katello_upgrade(
  ...
  get_targets($nodes).each |TargetSpec $node| {
    ...
    if $katello.name == $node.name {
      ...
      $current_version = Float($new_facter_result.first.value['stdout'].chomp())
      ...
      if $check_status.ok and ($current_version == $katello_version) {
        ...
        if ($katello_version > $metadata_version) and $update_metadata {
          ...
          # Print notification
          if ($katello_version > $metadata_version) {
            out::message("Revision: 'boltello::katello_version' updated from '${metadata_version}' to '${katello_version}' in $boltdir/data/common.yaml")
          }
        }
