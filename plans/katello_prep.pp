...
plan boltello::katello_prep(
  ...
  if !$major_packages_check.ok or $force {
    ...
    $puppetserver_package_version = $puppetserver_package_check.first.value['stdout']
    ...
    if $katello_role {
      ...
      if $db_manage {
        $rpm_version_query = run_command("/bin/rpm -qa --queryformat '%{version}' ${postgresql_prefix}postgresql",
          $nodes,
          'check postgresql version',
          _catch_errors => true
        )

        $postgresql_rpm_version = $rpm_version_query.first.value['stdout'] ? {
          /[0-9]/ => Float($rpm_version_query.first.value['stdout']),
          default => Float('0.0')
        }
      ...
      if ($postgresql_install or ($db_manage and $force)) {
        ...
        $installed_postgresql_version = $new_rpm_version_query.first.value['stdout'] ? {
          /[0-9]/ => Float($new_rpm_version_query.first.value['stdout']),
          default => Float('0.0')
        }
  ...
  # Remove postgresql and mongodb
  run_task('boltello::remove_packages', 
    $nodes, 
    packages => 'postgresql-server mongodb-server mongodb',
  )
}
